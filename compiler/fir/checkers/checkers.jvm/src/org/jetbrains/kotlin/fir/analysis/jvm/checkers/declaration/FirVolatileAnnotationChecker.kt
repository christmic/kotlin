...
object FirVolatileAnnotationChecker : FirPropertyChecker() {
    override fun check(declaration: FirProperty, context: CheckerContext, reporter: DiagnosticReporter) {
        ...
        val fieldAnnotation = declaration.backingField?.getAnnotationByClassId(VOLATILE_ANNOTATION_CLASS_ID, context.session)
        if (fieldAnnotation != null) {
            ...
            if (declaration.delegateFieldSymbol != null) {
                reporter.reportOn(fieldAnnotation.source, FirJvmErrors.VOLATILE_ON_DELEGATE, context)
            }
        }
    }
}