...
abstract class KotlinJsIrLibraryGradlePluginITBase : KGPBaseTest() {
    ...
    fun testSimpleJsBinaryLibrary(gradleVersion: GradleVersion) {
        project("simple-js-library", gradleVersion) {
            build("build") {
                assertFileInProjectExists("build/$DIST/productionLibrary/js-library.js")
                assertFileInProjectExists("build/$DIST/productionLibrary/package.json")
                assertFileInProjectExists("build/$DIST/productionLibrary/main.js")
                projectPath.resolve("build/$DIST/productionLibrary/package.json").reader()
    ...
    fun testJsBinaryLibraryAndExecutableForBrowserAndNodejs(gradleVersion: GradleVersion) {
        project("js-library-with-executable-browser-nodejs", gradleVersion) {
            build("build") {
                ...
                assertFileInProjectExists("build/$DIST/productionExecutable/js-library.js")
            }
        }
    }
}
